...
async def test_list_users_as_admin(client):
    response = await client.get("http://localhost:8006/admin/users", headers={"Authorization": ADMIN_TOKEN})
    assert response.status_code == 200
    assert isinstance(response.json(), list)

@pytest.mark.asyncio
async def test_block_user(client):
    response = await client.post("http://localhost:8006/admin/users/1/block", headers={"Authorization": ADMIN_TOKEN})
    assert response.status_code == 200
...
async def test_non_admin_block_forbidden(client):
    response = await client.post("http://localhost:8006/admin/users/1/block", headers={"Authorization": "Bearer not_admin"})
